...
MyMessage::MyMessage(const char *name, int kind) : ::cMessage(name,kind)
{
    this->noPass_var = 0;
    this->noLeftPass_var = 0;
}

MyMessage::MyMessage(const MyMessage& other) : ::cMessage(other)
{
    copy(other);
}
...
{
    this->noPass_var = other.noPass_var;
    this->noLeftPass_var = other.noLeftPass_var;
}

void MyMessage::parsimPack(cCommBuffer *b)
{
    ::cMessage::parsimPack(b);
    doPacking(b,this->noPass_var);
    doPacking(b,this->noLeftPass_var);
}

void MyMessage::parsimUnpack(cCommBuffer *b)
{
    ::cMessage::parsimUnpack(b);
    doUnpacking(b,this->noPass_var);
    doUnpacking(b,this->noLeftPass_var);
}

int MyMessage::getNoPass() const
{
    return noPass_var;
}

void MyMessage::setNoPass(int noPass)
{
    this->noPass_var = noPass;
}

int MyMessage::getNoLeftPass() const
{
    return noLeftPass_var;
}

void MyMessage::setNoLeftPass(int noLeftPass)
{
    this->noLeftPass_var = noLeftPass;
}
...
MyMessageDescriptor::MyMessageDescriptor() : cClassDescriptor("MyMessage", "cMessage")
{
}
...
int MyMessageDescriptor::findField(void *object, const char *fieldName) const
{
    ...
    if (fieldName[0]=='n' && strcmp(fieldName, "noPass")==0) return base+0;
    if (fieldName[0]=='n' && strcmp(fieldName, "noLeftPass")==0) return base+1;
...
std::string MyMessageDescriptor::getFieldAsString(void *object, int field, int i) const
{
    ...
    MyMessage *pp = (MyMessage *)object; (void)pp;
    switch (field) {
        case 0: return long2string(pp->getNoPass());
        case 1: return long2string(pp->getNoLeftPass());
        default: return "";
    }
}
...
bool MyMessageDescriptor::setFieldAsString(void *object, int field, int i, const char *value) const
{
    ...
    MyMessage *pp = (MyMessage *)object; (void)pp;
    switch (field) {
        case 0: pp->setNoPass(string2long(value)); return true;
        case 1: pp->setNoLeftPass(string2long(value)); return true;
